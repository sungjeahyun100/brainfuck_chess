-- Test-only Challenge clear persistence. Never touches prod.
BEGIN;
SELECT pg_advisory_xact_lock(hashtextextended('deck-chess-test-challenge-clears-v1', 0));
...
CREATE TABLE IF NOT EXISTS test.challenge_clears (
...
CREATE INDEX IF NOT EXISTS challenge_clears_user_first_cleared
    ON test.challenge_clears (user_id, first_cleared_at_ms DESC);
REVOKE ALL ON test.challenge_clears FROM PUBLIC, prod_app, test_app, deck_chess, deck_chess_test;
GRANT SELECT, INSERT ON test.challenge_clears TO test_app;
...
BEGIN
    IF EXISTS (
        ...
        WHERE conrelid = 'test.challenge_clears'::regclass AND contype = 'p'
          AND pg_get_constraintdef(oid) = 'PRIMARY KEY (user_id, challenge_id)'
    ) OR NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conrelid = 'test.challenge_clears'::regclass AND contype = 'f'
    ...
    ) OR NOT has_table_privilege('test_app', 'test.challenge_clears', 'SELECT')
       OR NOT has_table_privilege('test_app', 'test.challenge_clears', 'INSERT')
       OR has_table_privilege('test_app', 'test.challenge_clears', 'DELETE')
       OR has_table_privilege('prod_app', 'test.challenge_clears', 'SELECT') THEN
        RAISE EXCEPTION 'test.challenge_clears privilege verification failed';
    END IF;
END
$verify$;
RESET ROLE;
