-- Test game-record release. Run as the approved database administrator.
BEGIN;

SELECT pg_advisory_xact_lock(hashtextextended('deck-chess-test-create-game-records-v1', 0));

DO $admin_guard$
BEGIN
    IF session_user IN ('deck_chess', 'deck_chess_test') THEN
        RAISE EXCEPTION 'run the test game-record migration as the database administrator, not runtime role %', session_user;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_namespace WHERE nspname = 'test')
       OR NOT EXISTS (
        ...
        RAISE EXCEPTION 'shared/test schema split must be applied before game records';
    END IF;
...
BEGIN
    IF to_regclass('test.game_records') IS NOT NULL THEN
        ALTER TABLE test.game_records OWNER TO deck_chess_schema_owner;
    END IF;
END
...
CREATE TABLE IF NOT EXISTS test.game_records (
    id TEXT PRIMARY KEY,
...
BEGIN
    IF EXISTS (
        ...
        WHERE NOT EXISTS (
            ...
            WHERE actual.table_schema = 'test'
        ...
        RAISE EXCEPTION 'existing test.game_records does not match the required base contract';
    END IF;
END
$base_contract$;
...
BEGIN
    IF EXISTS (
        ...
        SELECT 1 FROM pg_constraint WHERE conrelid = 'test.game_records'::regclass
        ...
        SELECT 1 FROM pg_indexes WHERE schemaname = 'test' AND tablename = 'game_records'
       ...
       OR has_table_privilege('prod_app', 'test.game_records', 'SELECT')
       OR has_schema_privilege('prod_app', 'test', 'USAGE') THEN
        RAISE EXCEPTION 'test.game_records structure or privilege verification failed';
    END IF;
END
$verify$;
...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION 'temporary test migration owner membership was not removed';
    END IF;
END
