...
BEGIN
    IF current_user IN ('deck_chess', 'deck_chess_test') THEN
        RAISE EXCEPTION 'run the test game-record migration as the database administrator, not runtime role %', current_user;
    END IF;
    IF to_regnamespace('test') IS NULL OR to_regclass('shared.users') IS NULL THEN
