...
BEGIN
    IF session_user IN ('deck_chess', 'deck_chess_test') THEN
        RAISE EXCEPTION 'run the test ownership migration as the database administrator, not runtime role %', session_user;
    END IF;
...
-- administrator intentionally has no permanent test schema USAGE.
...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION 'temporary test schema-owner administrator membership was not removed';
    END IF;
END
