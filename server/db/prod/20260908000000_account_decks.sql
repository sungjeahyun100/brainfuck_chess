-- Production-only Account deck persistence. Apply with the approved admin identity.
BEGIN;
SELECT pg_advisory_xact_lock(hashtextextended('deck-chess-prod-account-decks-v1', 0));
...
CREATE TABLE IF NOT EXISTS prod.decks (
    id TEXT PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS decks_owner_updated ON prod.decks (owner_id, updated_at_ms DESC, id);
REVOKE ALL ON prod.decks FROM PUBLIC, prod_app, test_app, deck_chess, deck_chess_test;
GRANT SELECT, INSERT, UPDATE, DELETE ON prod.decks TO prod_app;
DO $verify$
BEGIN
    IF EXISTS (
        ...
        WHERE NOT EXISTS (SELECT 1 FROM information_schema.columns actual
            WHERE table_schema='prod' AND table_name='decks' AND actual.column_name=expected.column_name
              AND actual.data_type=expected.data_type AND is_nullable='NO')
    ) OR NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid='prod.decks'::regclass
      ...
      OR EXISTS (SELECT 1 FROM (VALUES ('SELECT'),('INSERT'),('UPDATE'),('DELETE')) p(privilege)
        WHERE NOT has_table_privilege('prod_app', 'prod.decks', p.privilege)
           OR has_table_privilege('test_app', 'prod.decks', p.privilege))
      OR (SELECT pg_get_userbyid(relowner) FROM pg_class WHERE oid='prod.decks'::regclass) <> 'deck_chess_schema_owner'
    THEN RAISE EXCEPTION 'prod.decks shape, owner or permission contract is invalid'; END IF;
END
$verify$;
RESET ROLE;
