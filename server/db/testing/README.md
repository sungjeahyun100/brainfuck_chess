...
2. Run the shared profile-visibility SQL and the test/prod game-record creation
   and ownership SQL (through `20260826001000_game_record_ownership.sql`).
...
It provisions the fixture, applies the legacy ownership migrations twice and
verifies them before retention changes the permissions. It then runs the current
test and prod releases twice, runs the rollback-only environment isolation probe,
and verifies account decks. The fixture creates cluster-level roles, so never use an existing
