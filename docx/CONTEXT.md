...
* 게임 중 포켓 기물 착수
* 한 턴에 기물 이동 1회 또는 포켓 기물 착수 1회만 허용하는 단일 행동 턴
* 체크 / 체크메이트 없음
...
기본 체스의 기물 이름과 직관성은 유지하되, 덱 빌딩, 포켓 착수, 확장 보드를 통해 전략 복잡도를 높인다.
...
BrainfuckChessRuleEngine = 게임 전체 규칙, 턴, 착수, 승패 판정
```
...
한 턴에는 기물 이동 1회 또는 포켓 기물 착수 1회 중 하나만 수행한다.

규칙:

* 이동을 선택했다면 자신의 보드 위 기물 1개를 1회 이동한다.
* 착수를 선택했다면 포켓 기물 1개를 보드에 1회 놓는다.
...
* 앙파상은 직전 상대 이동에만 응답할 수 있는 예외 규칙이다.
...
* 이동 / 착수 횟수 제한, 턴 종료, King 포획, 게임 종료를 처리한다.
...
* Brainfuck Chess 룰 엔진은 Chessembly 결과를 받아 턴, 착수, 포획, 승패를 처리한다.
