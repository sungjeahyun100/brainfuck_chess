...
export function newCustomPieceScript(): string {
  return serializeCustomPiecePackage({ format: 'brainfuck-chess-piece-set-v1', definitions: [newCustomPieceDefinition()] })
}
...
export function newAdvancedCustomPieceDraft(source: SimpleCustomPieceDraft = newSimpleCustomPieceDraft()): AdvancedCustomPieceDraft {
  const input = buildCustomPieceInput(source)
  return {
    name: source.name, description: source.description, score: source.score,
    image: { ...source.image }, rawScript: input.raw_script, exposedPieceKey: input.exposed_piece_key,
  }
}
...
export function buildCustomPieceInput(draft: SimpleCustomPieceDraft): CustomPieceInput {
  ...
  definition.state_schema = draft.abilities.map((ability, index) => ({
    key: safeStateKey(ability.name, index), default_value: ability.initialValue,
  }))
  return {
    name: draft.name, description: draft.description, score: draft.score, image: { ...draft.image },
    raw_script: serializeCustomPiecePackage({ format: 'brainfuck-chess-piece-set-v1', definitions: [definition] }),
...
export function buildAdvancedCustomPieceInput(draft: AdvancedCustomPieceDraft): CustomPieceInput {
  ...
  return {
    name: draft.name, description: draft.description, score: draft.score, image: { ...draft.image },
    raw_script: serializeCustomPiecePackage(document), exposed_piece_key: draft.exposedPieceKey,
  }
}
...
export function advancedDraftFromInput(input: CustomPieceInput): AdvancedCustomPieceDraft {
  ...
  return {
    name: input.name, description: input.description, score: input.score,
    image: { ...input.image }, rawScript: input.raw_script, exposedPieceKey: input.exposed_piece_key,
  }
}
...
export function simpleDraftFromInput(input: CustomPieceInput): SimpleCustomPieceDraft | null {
  ...
  try { document = parseCustomPiecePackage(input.raw_script) } catch { return null }
  ...
  return {
    name: input.name, description: input.description, score: input.score, image: { ...input.image },
    movementCode: definition.chessembly_code,
    abilities: definition.state_schema.map(state => ({
      kind: 'remember_value', name: state.key, initialValue: state.default_value as number,
    })),
  }
}
...
export function parseCustomPiecePackage(rawScript: string): CustomPiecePackageDocument {
  ...
  if (!Array.isArray(document.definitions) || document.definitions.length === 0) throw new Error('기물 정의가 하나 이상 필요합니다.')
  return {
    format: document.format,
    definitions: document.definitions.map((definition, index) => normalizeCustomPieceDefinition(definition, index)),
  }
}
...
function normalizeCustomPieceDefinition(value: PieceDefinition, index: number): PieceDefinition {
  ...
  return {
    ...
    is_king: false, can_capture_on_drop: false,
  }
}
...
export function customPieceTemplate(kind: AdvancedTemplateKind): CustomPiecePackageDocument {
  ...
  if (kind === 'windmill') {
    ...
    definition.move_layers = [
      { id: 'bishop_mode', chessembly_code: BISHOP_CODE, enabled_when: [{ key: 'mode', condition: { equals: 'bishop' } }], on_commit: [{ key: 'mode', value: 'rook' }] },
      { id: 'rook_mode', chessembly_code: ROOK_CODE, enabled_when: [{ key: 'mode', condition: { equals: 'rook' } }], on_commit: [{ key: 'mode', value: 'bishop' }] },
    ]
    definition.move_options = [{ id: 'normal', name: '일반 이동', description: '이동할 때마다 비숍/룩 모드를 전환합니다.', kind: 'normal', layer_ids: ['bishop_mode', 'rook_mode'], execution_mode: 'move_modifier', contributes_to_attack_map: true }]
    definition.visual = { default_asset_key: 'bishop', variants: [{ id: 'rook_mode', enabled_when: [{ key: 'mode', condition: { equals: 'rook' } }], asset_key: 'rook', priority: 10 }] }
    ...
    definition.move_options = [
      { id: 'normal', name: '일반 이동', description: '', kind: 'normal', layer_ids: ['rook_move'], execution_mode: 'move_modifier', contributes_to_attack_map: true },
      { id: 'cannon_move', name: '포 이동', description: '기물 하나를 뛰어넘는 이동입니다.', kind: 'ability', layer_ids: ['cannon_move'], execution_mode: 'move_modifier', contributes_to_attack_map: true, cooldown: { turns: 3, clock: 'owner_turns' } },
    ]
    ...
    definition.move_options = [
      { id: 'normal', name: '일반 이동', description: '', kind: 'normal', layer_ids: ['bishop_move'], execution_mode: 'move_modifier', contributes_to_attack_map: true },
      { id: 'bounce_move', name: '반사 이동', description: '가장자리에서 반사되는 이동입니다.', kind: 'ability', layer_ids: ['bounce_move'], execution_mode: 'move_modifier', contributes_to_attack_map: true, cooldown: { turns: 2, clock: 'owner_turns' } },
    ]
...
export function customPieceDraftSnapshot(draft: unknown): string { return JSON.stringify(draft) }
...
export function validateAdvancedCustomPieceDraft(draft: AdvancedCustomPieceDraft): string {
  ...
  try { buildAdvancedCustomPieceInput(draft) }
  catch (error) { return error instanceof Error ? error.message : '고급 기물 정의가 올바르지 않습니다.' }
  return ''
}
...
export function testPiecesFromServerState(state: GameState): CustomPieceTestPiece[] {
  return Object.values(state.pieces)
    ...
    .map(piece => ({
      ...
      owner: piece.owner, square: piece.current_square!, state: piece.state,
    }))
}
