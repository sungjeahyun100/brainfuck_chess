<template>
  <section v-if="state.ruleset === 'standard'" class="reserve-panel" :aria-label="`${side === 'white' ? '백' : '흑'} Hand / Pocket`">
    ...
    <details v-if="revealPocket"><summary>Pocket: {{ pocket.length }} · 드로우 및 능력용</summary>
...
}>(), { revealPocket: false, enabled: false, selectedId: null, summoning: false, candidates: () => [], selectedSacrifices: () => [] })
...
p { margin:6px 0; }
.reserve-list { display:grid; gap:6px; overflow:auto; padding:6px 2px; max-height:220px; }
button, .pocket-item { flex:0 0 auto; display:flex; align-items:center; gap:6px; border:1px solid #53657d; border-radius:6px; background:#253247; color:inherit; padding:5px; font-size:12px; line-height:1.2; }
button { cursor:pointer; } button:disabled { opacity:.6; cursor:default; }
button small { display:block; } img { width:28px; height:28px; object-fit:contain; }
