    ...
    <div class="replay-layout">
      ...
      <aside class="replay-sidebar">
        <section><h3>기보</h3><div class="notation-list">
          ...
          <button v-for="entry in notation" :key="entry.ply" class="notation-row" :class="{ active: ply === entry.ply }" @click="go(entry.ply)">
            <span>{{ entry.ply }}. {{ entry.text }}</span><small>{{ duration(entry.elapsed) }}</small>
          </button>
        </div></section>
...
import { encodeReplayCode, replayHashesMatch } from '../replayCodec'
...
const state = computed(() => ply.value === 0 ? props.record.initial_state : props.record.actions[ply.value - 1].state_after)
const activeClock = computed(() => ply.value === 0 ? props.record.initial_clock : props.record.actions[ply.value - 1].clock)
const notation = computed(() => props.record.actions.map(entry => ({ ply: entry.ply, text: formatAction(entry.action, entry.state_after), elapsed: entry.elapsed_ms })))
const hashMismatch = ref(false)
const compatibilityWarning = computed(() => hashMismatch.value || props.record.format_version !== 1 || props.record.ruleset_version !== 'deck-chess-1' || props.record.chessembly_version !== 'chessembly-1')
...
onMounted(async () => { window.addEventListener('keydown', keydown); hashMismatch.value = !await replayHashesMatch(props.record) }); onUnmounted(() => { stop(); window.removeEventListener('keydown', keydown) })
</script>
