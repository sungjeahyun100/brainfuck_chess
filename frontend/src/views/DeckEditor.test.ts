...
test('wizard queen and rook can be found, placed, and added to the pocket', async t => {
  ...
  for (const ruleset of ['legacy', 'standard']) {
    ...
    for (const [id, name] of [['wizard-queen', '마법사 퀸'], ['wizard-rook', '마법사 룩']]) {
      ...
      assert.equal(entries[0].deploymentZone, 'back')
      state.deck.value.starting = []
      state.deck.value.pocket = {}
      state.placementTool.value = id
      const square = validation.backZoneSquares(12, 'white', ruleset as 'legacy' | 'standard')[0]
      state.onPlacementSquareClick(square.file, square.rank)
      assert.deepEqual(state.deck.value.starting, [{ pieceType: id, square }])
      state.onPocketClick()
      assert.equal(state.deck.value.pocket[id], 1)
      assert.equal(validation.pocketCatalog.some(piece => piece.id === id), true)
    }
  }
})
